...
func Test_Router_OIDCLogin_Redirect(t *testing.T) {
	...
	testURL := fmt.Sprintf("/federated/oidc?redirectUrl=%s", url.QueryEscape(redirect))
...
func Test_Router_OIDCLogin_Complete(t *testing.T) {
	...
	testCompleteURL := fmt.Sprintf("http://localhost:8081/federated/oidc/complete?code=%s&state=%s", "test", state)
